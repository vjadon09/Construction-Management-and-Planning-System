...
import qualityOfLifeMS_pb2
import qualityOfLifeMS_pb2_grpc
import pandas
import grpc
import threading
...
vals = df.values
class main(qualityOfLifeMS_pb2_grpc.qualityOfLifeMSServicer):
    ...
    def getEnv(self, request, context):
        ...
        return qualityOfLifeMS_pb2.ranking(rankings = str(self.getRanking(0)), response = "NOT IMPLEMENTED YET!") 
    


    def getWalk(self, request, context):
        self.updateLists(request.locations)
        return qualityOfLifeMS_pb2.ranking(rankings = str(self.getRanking(1)), response = "NOT IMPLEMENTED YET!") 



    def getBoth(self, request, context):
        self.updateLists(request.locations)
        return qualityOfLifeMS_pb2.ranking(rankings = str(self.getRanking(0.5)), response = "NOT IMPLEMENTED YET!") 

def serve(): #GRPC stuff.
    print("starting server")
    server = grpc.server(concurrent.futures.ThreadPoolExecutor(max_workers=10))
    qualityOfLifeMS_pb2_grpc.add_qualityOfLifeMSServicer_to_server(main(), server)
