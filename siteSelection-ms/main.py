...
df = pandas.read_csv('sitesData.csv')
vals = df.values


class main(siteSelectionMS_pb2_grpc.siteSelectionMSServicer):
    ...
    def getCommute(self, request, context):
        ...
        return siteSelectionMS_pb2.rankedProxResponse(rankedAreas=str(self.getRanking(0)), rankings=1, rankedResponse="NOT IMPLEMENTED YET!")

    def getDrive(self, request, context):
        self.updateLists(request.locations)
        return siteSelectionMS_pb2.ranking(rankings={str(self.getRanking(1))}, response="NOT IMPLEMENTED YET!")

    def getCommDrive(self, request, context):
        self.updateLists(request.locations)
        return siteSelectionMS_pb2.ranking(rankings={str(self.getRanking(0.5))}, response="NOT IMPLEMENTED YET!")


def serve():  # GRPC stuff.
    print("starting server")
    server = grpc.server(concurrent.futures.ThreadPoolExecutor(max_workers=10))
    siteSelectionMS_pb2_grpc.add_siteSelectionMSServicer_to_server(main(), server)
